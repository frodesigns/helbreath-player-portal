<?php
	...
	$result = QueryWS1("SELECT PostID FROM PPPosts ORDER BY PostID ASC");
	
	for ( $i = 0; $i < Num_Rows($result); $i++ ) {
		$postid = Result($result, $i, "PostID");
		
		$result11 = QueryWS1("SELECT COUNT(*) FROM PPPostVotes WHERE Vote = 1 AND PostID = $postid");						
		list($upvotes) = mssql_fetch_row($result11);

		$result12 = QueryWS1("SELECT COUNT(*) FROM PPPostVotes WHERE Vote = 0 AND PostID = $postid");						
		list($downvotes) = mssql_fetch_row($result12);
		
		$rating = $upvotes - $downvotes;
		
		$result13 = QueryWS1("UPDATE PPPosts SET PostRating = $rating WHERE PostID = $postid");
	}

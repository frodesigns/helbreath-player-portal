<?php
	...
	$queryares = "select top 30 ch.CharID, ch.cCharName, ch.sLevel, sk.sSkillMastery, 'active' =  CASE WHEN ch.FileSaveDate > GETDATE()-30 THEN 'Yes' ELSE 'No' END
		from ws1.dbo.character_t as ch, mainlogin.dbo.account_t as ml,  ws1.dbo.skill_t as sk
		where (ch.cNation='aresden') and ml.cAccountID = ch.cAccountID and ch.CharID = sk.CharID and sk.sSkillID = 1 and
	...
	$queryelv = "select top 30 ch.CharID, ch.cCharName, ch.sLevel, sk.sSkillMastery, 'active' =  CASE WHEN ch.FileSaveDate > GETDATE()-30 THEN 'Yes' ELSE 'No' END
		from ws1.dbo.character_t as ch, mainlogin.dbo.account_t as ml,  ws1.dbo.skill_t as sk
		where (ch.cNation='elvine') and ml.cAccountID = ch.cAccountID and ch.CharID = sk.CharID and sk.sSkillID = 1 and
